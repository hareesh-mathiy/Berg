...
import gameobjects.PenguinBounds;
...
public class LevelLoader {
    ...
    public LevelLoader(TiledMap tiledMap, GameRenderer r){
        ...
        world = r.getWorld();
        isaac = r.getIsaac();
        penguin = r.getPenguin();
        camera = r.getCam();
        pos = new Vector3(0, camera.position.y/2, 0);

        ani = new Animations(batch, isaac, penguin, r);
    ...
    public void render(float runTime){
        //ani.playIntro(runTime);
        //if(AssetLoader.introani.isAnimationFinished(runTime)){
        //    if(AssetLoader.introrepeatani.isAnimationFinished(runTime) && !introFinished){
        //        ani.playIntroRepeat(runTime);
        //        if(Gdx.input.isTouched()) {
        //            ani.shakeIntroRepeat(runTime);
        //            crack = true;
        //        }
        //        if(crack){
        //
        ...
        //    else{
                ...
                if(ani.psyActive){
                    ...
                    renderOverWalkable();
                    batch.begin();
                    batch.setColor(ani.getRandColor());
                    ...
                    else if(isaac.getHPos() == 0){
                        ani.run(runTime);
                        renderOver();
                        renderWalkable();
                        renderOverWalkable();
                        batch.begin();
                        ani.getPe().draw(batch);
                        ani.getPeP().draw(batch);
                        ani.getShadowP().draw(batch);
                        ani.drawPenguin(runTime);
                        batch.end();
                    }
                }
                batch.begin();
                overlay.draw(batch);
                ani.getSnow().draw(batch);
                ani.getOverlay().draw(batch);
                batch.end();

                if(isaac.falling){
                    isaac.b2body.setLinearVelocity(0,-25);
                }
    ...
    public void createObjects(){
        ...
        for(MapObject object : tiledMap.getLayers().get(9).getObjects()) {
            if (object instanceof TextureMapObject) {
                continue;
            }
            Shape shape;
            ...
            new PenguinBounds(world, tiledMap, shape, this, object);
        }
    ...
    public boolean introFinished(){ return introFinished; }
}
